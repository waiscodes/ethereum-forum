...
impl SSOService {
    pub async fn new(figment: Figment) -> Result<Self> {
        ...
        let jwt_expiry_hours: Option<i64> = env_config.extract_inner("jwt_expiry_hours").ok();
        ...
        if typed_providers.is_empty() {
            return Err(anyhow!(
                "No SSO providers configured. Please set SSO_PROVIDERS__<provider>__* environment variables"
            ));
        }
    ...
    pub async fn exchange_code_for_token(
        ...
        let token_response = self
            .exchange_code_for_access_token(provider_config, code)
            .await?;
        ...
        let sso_user_info = UserInfo {
            ...
            email: user_info
                .email
                .clone()
                .unwrap_or_else(|| format!("{}@{}", user_info.sub, provider_id)),
            name: user_info
                .name
                .clone()
        ...
        if let Some(preferred_username) = &user_info.preferred_username {
            extras.insert(
                ...
                serde_json::Value::String(preferred_username.clone()),
            );
        }
        extras.insert(
            "provider_user_info".to_string(),
    ...
    pub fn validate_jwt_token(&self, token: &str) -> Result<JWTClaims> {
        ...
        match decode::<JWTClaims>(token, &self.jwt_decoding_key, &validation) {
            ...
            Err(e) => {
                // Only log specific error details on higher log levels
                match e.kind() {
                    ...
                    jsonwebtoken::errors::ErrorKind::InvalidToken => {
                        tracing::debug!("JWT token format invalid");
                    }
                    _ => {
                        tracing::debug!("JWT validation error: {:?}", e.kind());
                    }
                }
